...
def grid_visibilities_parallel(
    ...
    print(f"parallel gridded visgrid shape: {visgrid.shape}")
    return visgrid, kernel_weights
...
def frequency_fft(vis, freq, dim, taper=signal.blackmanharris, n_obs=1):
    ...
    print(etaz.shape, "etaz.shape")
    return np.array(ft), np.array(etaz)  # eta_coords
...
def Bestimator(
    ...
    if n_obs == 1:
        ...
        print("GRIDDING IN PARALLEL MODE")
    ...
    if n_obs == 1:
        selection = (len(eta_coords) // 2) + 1
        print(selection, "selection")
        eta_coords = eta_coords[selection:]
        power2D = power2D[0][:, selection:]
    else:
        selection = len(eta_coords) // 2
        print(selection, "selection")
        eta_coords = eta_coords[selection:]
        print(power2D.shape, "unstacked power2D SHAPE before selection")
        power2D = np.hstack(power2D)
        print(power2D.shape, "stacked power2D SHAPE")

    print(power2D.shape, "final power2D SHAPE")
